...
| Total calls | | |
| Distinct tools | | |
| **Knowledge calls** (`search`, `symbol`, `graph`, `module_docs`, `deprecations`, `entities`, `routes`, `capabilities`, `architecture`, `services`, `db_schema`, `wiki`) | | **zero here means the run never asked the codebase anything**, whatever its grounding table says |
